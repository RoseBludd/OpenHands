...
- **Agents-first sidebar**: conversations group by launched agent profile (`organizeMode: "agents"`, the default; per-twin folders with per-agent new-chat launch). New UI strings need 15-locale entries.
- **Agents-first sidebar** (2026-09-01): the sidebar groups conversations by AgentProfile (`archy.`, `devvy.`, ...) by default (`organizeMode: "agents"`, persisted v1 migration). Every known profile gets a folder even with zero threads (Grok-style always-listed); each folder has a "Start new conversation" launch that opens a chat under that twin; profile-less threads fall into "Unassigned". Deployed at agents.geniuzs.com/canvas from `bc0d142`. Inline media: twin `file_publish` output URLs (public R2 `pub-*.r2.dev`) render as inline images/video in chat; PDFs open in a tab.
