...
- **Agents-first sidebar** (2026-09-01): the sidebar groups conversations by AgentProfile (`archy.`, `devvy.`, ...) by default (`organizeMode: "agents"`, persisted v1 migration). Each agent folder has a launch button that starts a new chat with that twin; profile-less threads fall into "Unassigned". Inline media: twin `file_publish` output URLs (public R2 `pub-*.r2.dev`) render as inline images/video in chat; PDFs open in a tab.
